  ...
  <el-row>
    <el-col :span="4">
      <el-menu class="el-menu-demo">
        <el-menu-item v-for="chatRoom in allChatRoom" :key="chatRoom.chatRoomId" @click="enterChatRoom(chatRoom.chatRoomId)">
    ...
    <el-col :span="20" :push="1">
      ...
      <div id="fuck" class="chatWindow">
        ...
        <el-row :class="nowChat.Sender===userId?'sendChat-Row':'getChat-Row'">
          <div :class="nowChat.Sender===userId?'sendChat-dialog':'getChat-dialog'">
            {{nowChat.Content}}
          </div>
        </el-row>
        <el-row :class="nowChat.Sender===userId?'sendChat-timeSpan':'getChat-timeSpan'">
...
export default class chatView extends Vue {
  ...
  //使用者所有聊天室
  private allChatRoom :{chatRoomId:string,chatRoomName:string,chatRoomContent:string,UpdateTime:string}[]=[];
  //全部資料
  private entireData :[]=[];
  ...
  private async enterChatRoom(chatRoomId:any): Promise<void> {
    ...
    this.nowChatArray = [...myChatRoom['chat']].sort((a, b) => (a['ChatTime'] < b['ChatTime'] ? 1 : -1));
    //判斷現在在跟誰說話
    this.talkTo = myChatRoom['Member'][myChatRoom['Member'][0]===this.userId?1:0]
    //指定現在的聊天室Id 
